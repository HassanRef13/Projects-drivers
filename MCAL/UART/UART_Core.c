...
void UART_Init(void)
{
	...
	/********** Parity **************/
		...
		#if (UART_Parity_STATE == POLARITY_ODD)
		UCSRC |= 0XD0;
		#elif (UART_POLARITY_TYPE == POLARITY_EVEN)
		UCSRC |= 0XA0;
		#endif
...
{
	// check if the all data sent
	// smarter way  -> if (UART_TRANSMITE_DATA[TX_CNT] == '.') delemeter
...
void UART_ReceiveData(void)
{
	...
	// increase the counter as the message did not end yet
	Receive_Counter++;
	// check for the delemeter (end of the message)
		if (UDR == '.' )
		{
			Receive_Counter=0;
